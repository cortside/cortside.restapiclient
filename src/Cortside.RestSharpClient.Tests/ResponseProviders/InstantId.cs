...
namespace Cortside.RestSharpClient.Tests.Mocks {
    public static class InstantId {
        public static string BuildResponse(string requestBody) {
            ...
            userRequest = System.Uri.UnescapeDataString(userRequest);
        ...
        private static string GetCustomRiskCodeXmlForResponse(string userRequest) {
            ...
            foreach (var code in codes.Split(",")) {
                if (!string.IsNullOrEmpty(code.Trim())) {
                    riskCodeXml += $"<RiskIndicator><RiskCode>{code.ToUpper().Trim()}</RiskCode><Description>Generated by EnerBank's LexisNexis.WireMock</Description><Sequence>{sequence}</Sequence></RiskIndicator>";
                    sequence++;
                }
            }
