        ...
        private static string GetCustomRiskCodeXmlForResponse(string userRequest) {
            ...
            foreach (var code in codes.Split(",")) {
                if (!string.IsNullOrEmpty(code.Trim())) {
                    riskCodeXml += $"<RiskIndicator><RiskCode>{code.ToUpper().Trim()}</RiskCode><Description>Generated by LexisNexis.WireMock</Description><Sequence>{sequence}</Sequence></RiskIndicator>";
                    sequence++;
                }
            }
