        ...
        public void Configure(MockHttpServer server) {
            server.WireMockServer
                .Given(
                    Request.Create()
                        ...
                        .WithBody(b => b?.Contains("grant_type=delegation") == true && b.Contains("client_id=foo"))
                        .UsingPost()
                )
                .RespondWith(
                    Response.Create()
            ...
            server.WireMockServer
                .Given(
                    Request.Create()
                        .WithPath("/connect/token")
                        .WithBody(b => b?.Contains("grant_type=client_credentials") == true && b.Contains("client_id=foo"))
                        .UsingPost()
                )
                .RespondWith(
                    Response.Create()
            ...
            server.WireMockServer
                .Given(
                    Request.Create()
                        .WithPath("/connect/token")
                        .WithBody(b => b?.Contains("grant_type=client_credentials") == true && b.Contains("client_id=allow"))
                        .UsingPost()
                )
                .RespondWith(
                    Response.Create()
